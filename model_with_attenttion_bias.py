...
class AttentionBias(nn.Module):
    """Learnable attention bias for 2D images with efficient implementation"""
    def __init__(self, num_heads, max_height=64, max_width=64):
        super().__init__()
        ...
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros(num_heads, (2 * max_height - 1) * (2 * max_width - 1))
        )
        
        # Initialize with truncated normal
        nn.init.trunc_normal_(self.relative_position_bias_table, std=0.02)
        
        # Create relative position index for efficient lookup
        coords_h = torch.arange(max_height)
        coords_w = torch.arange(max_width)
        ...
        relative_coords[0] += max_height - 1  # Height dimension
        relative_coords[1] += max_width - 1   # Width dimension
        
        # Flatten the relative coordinates
        relative_coords[0] *= 2 * max_width - 1
        relative_position_index = relative_coords.sum(0)  # [H*W, H*W]
    ...
    def forward(self, attn_scores, h, w):
        """
        Apply learnable attention bias to attention scores
        Args:
        ...
        if h > self.max_height or w > self.max_width:
            raise ValueError(
                f"Feature map size {h}x{w} is larger than maximum supported size "
                f"{self.max_height}x{self.max_width}"
            )
            
        # Get bias values using the precomputed indices
        bias_index = self.relative_position_index[:h*w, :h*w].reshape(-1)
        bias = self.relative_position_bias_table.view(self.num_heads, -1)[:, bias_index]
...
class MultiHeadSelfAttention(nn.Module):
    """Multi-head self-attention with learnable attention bias"""
    def __init__(self, dim, num_heads=8):
        ...
        self.attention_bias = AttentionBias(num_heads, max_height=64, max_width=64)
    ...
    def forward(self, x):
        ...
        attn = self.attention_bias(attn, H, W)
...
class Generator(nn.Module):
    ...
    def __init__(self, conv_dim=64, c_dim=5, repeat_num=6,device = 'cuda:6',num_memory = 4):
        ...
        # Bottleneck layers with self-attention and learnable attention bias
        layers = []
