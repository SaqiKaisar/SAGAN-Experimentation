...
class Generator(nn.Module):
    ...
    def __init__(self, conv_dim=64, c_dim=5, repeat_num=6,device = 'cuda:6',num_memory = 4):
        ...
        self.positional_dim = int(np.log2(self.num_memory) + 1)  # You can increase this if desired
        self.position_embedding = nn.Embedding(self.num_memory, self.positional_dim)
    ...
    def add_condition(self, x, bs, num_windows):
        """
        Append learned positional embeddings to feature maps.
        x: (bs, num_windows, C, H, W)
        returns: (bs, num_windows, C + pos_dim, H, W)
        """
        device = x.device
        # Get learned embeddings for memory indices
        indices = torch.arange(num_windows, device=device)  # [0, 1, ..., num_memory-1]
        pos_enc = self.position_embedding(indices)  # shape: (num_windows, pos_dim)
        
        # Reshape to broadcast over (bs, H, W)
        pos_enc = pos_enc.view(1, num_windows, self.positional_dim, 1, 1)
        pos_enc = pos_enc.expand(bs, -1, -1, x.size(-2), x.size(-1))  # shape: (bs, num_windows, pos_dim, H, W)
